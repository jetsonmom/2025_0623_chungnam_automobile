...
# 2. 📊 자율주행 자료구조에서 사용되는 리스트, 딕셔너리, 튜플 비교
## 📊 자료구조 비교표

| **자료구조** | **특징** | **자율주행 예시** |
|-------------|----------|------------------|
| **딕셔너리** | • 서로 다른 종류의 데이터를 의미 있는 이름으로 묶어서 관리<br>• 관련 데이터를 묶어서 관리<br>• 코드 가독성이 좋아짐<br>• `sensors["앞"]`으로 앞쪽 센서값을 바로 알 수 있음 | `vehicle_speed = {"current": 45.0, "limit": 60.0}` |
| **리스트** | • 같은 종류의 데이터가 여러 개 있을 때 적합<br>• 순서가 중요할 때 - 시간순 데이터, 좌표 순서<br>• 같은 성격의 데이터 여러 개 - 타이어 4개, 센서 여러 개<br>• 인덱스로 접근 - `tire_pressures[0]` (첫 번째 타이어) | `tire_pressures = [2.2, 2.1, 2.3, 2.2]` |
